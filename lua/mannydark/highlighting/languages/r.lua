...
r.setupHighlighting = function()
  ...
  highlight(0, 'rConditional',      { fg = colors.blue,       bg = 'NONE' })  -- if, else
  highlight(0, 'rRepeat',           { fg = colors.blue,       bg = 'NONE' })  -- for, in, repeat, while
  highlight(0, 'rKeyword',          { fg = colors.blue,       bg = 'NONE' })  -- function, etc.
  highlight(0, 'rFunction',         { fg = colors.blue,       bg = 'NONE' })  -- function keyword

  -- Constants
  highlight(0, 'rConstant',         { fg = colors.blue,       bg = 'NONE' })  -- NULL, NA, Inf, NaN, TRUE, FALSE
  highlight(0, 'rBoolean',          { fg = colors.blue,       bg = 'NONE' })  -- TRUE, FALSE, T, F
  highlight(0, 'rPreProc',          { fg = colors.blue,       bg = 'NONE' })  -- library, require, attach, detach, source

  -- Types
  highlight(0, 'rType',             { fg = colors.turquoise,  bg = 'NONE' })  -- array, character, complex, double, function, etc.

  -- Functions
  highlight(0, 'rFunctionName',     { fg = colors.orange,     bg = 'NONE' })  -- Function name definitions
  highlight(0, 'rMethodCall',       { fg = colors.orange,     bg = 'NONE' })  -- Method calls

  -- Operators
  highlight(0, 'rOperator',         { fg = colors.white,      bg = 'NONE' })  -- +, -, *, /, etc.
  highlight(0, 'rAssign',           { fg = colors.white,      bg = 'NONE' })  -- <-, ->, =, <<-, ->>
  highlight(0, 'rPipeOperator',     { fg = colors.blue,       bg = 'NONE' })  -- |>, %>%
  highlight(0, 'rTilde',            { fg = colors.blue,       bg = 'NONE' })  -- ~ formula operator
  highlight(0, 'rInfixOperator',    { fg = colors.blue,       bg = 'NONE' })  -- %...% operators
  ...
  -- Strings
  highlight(0, 'rString',           { fg = colors.redLight,   bg = 'NONE' })  -- "string" or 'string'
  highlight(0, 'rRawString',        { fg = colors.redLight,   bg = 'NONE' })  -- r"(...)" raw strings
  highlight(0, 'rRawStrDelim',      { fg = colors.redLight,   bg = 'NONE' })  -- Raw string delimiters
  highlight(0, 'rSpecial',          { fg = colors.pink,       bg = 'NONE' })  -- Escape sequences \n, \t, etc.
  highlight(0, 'rStrError',         { fg = colors.red,        bg = 'NONE' })  -- String errors

  -- Numbers
  highlight(0, 'rNumber',           { fg = colors.greenLight, bg = 'NONE' })  -- Numbers
  ...
  -- Comments
  highlight(0, 'rComment',          { fg = colors.red,        bg = 'NONE' })  -- # comments
  highlight(0, 'rCommentTodo',      { fg = colors.red,        bg = 'NONE', bold = true })  -- TODO, FIXME, etc.
  ...
  highlight(0, 'rDelimiter',        { fg = colors.white,      bg = 'NONE' })  -- (), {}, []
  ...
  highlight(0, 'rOComment',         { fg = colors.red,        bg = 'NONE' })  -- #' roxygen comments
  highlight(0, 'rOCommentKey',      { fg = colors.pink,       bg = 'NONE' })  -- @param, @return, etc.
  ...
  -- Variables
  highlight(0, '@variable.r',              { fg = colors.white,     bg = 'NONE' })  -- Variables
  highlight(0, '@variable.builtin.r',      { fg = colors.blue,      bg = 'NONE' })  -- Built-in variables
  highlight(0, '@variable.parameter.r',    { fg = colors.purple,    bg = 'NONE' })  -- Function parameters
  highlight(0, '@variable.member.r',       { fg = colors.purple,    bg = 'NONE' })  -- List/data frame members

  -- Constants
  highlight(0, '@constant.r',              { fg = colors.purple,    bg = 'NONE' })  -- Constants
  highlight(0, '@constant.builtin.r',      { fg = colors.blue,      bg = 'NONE' })  -- NULL, NA, TRUE, FALSE, Inf, NaN

  -- Functions
  highlight(0, '@function.r',              { fg = colors.orange,    bg = 'NONE' })  -- Function definitions
  highlight(0, '@function.call.r',         { fg = colors.orange,    bg = 'NONE' })  -- Function calls
  highlight(0, '@function.builtin.r',      { fg = colors.orange,    bg = 'NONE' })  -- Built-in functions
  highlight(0, '@function.method.r',       { fg = colors.orange,    bg = 'NONE' })  -- Method definitions
  highlight(0, '@function.method.call.r',  { fg = colors.orange,    bg = 'NONE' })  -- Method calls

  -- Types
  highlight(0, '@type.r',                  { fg = colors.turquoise, bg = 'NONE' })  -- Types
  highlight(0, '@type.builtin.r',          { fg = colors.turquoise, bg = 'NONE' })  -- Built-in types

  -- Keywords
  highlight(0, '@keyword.r',               { fg = colors.blue,      bg = 'NONE' })  -- General keywords
  highlight(0, '@keyword.function.r',      { fg = colors.blue,      bg = 'NONE' })  -- function
  highlight(0, '@keyword.return.r',        { fg = colors.blue,      bg = 'NONE' })  -- return
  highlight(0, '@keyword.conditional.r',   { fg = colors.blue,      bg = 'NONE' })  -- if, else
  highlight(0, '@keyword.repeat.r',        { fg = colors.blue,      bg = 'NONE' })  -- for, while, repeat
  highlight(0, '@keyword.operator.r',      { fg = colors.blue,      bg = 'NONE' })  -- in
  highlight(0, '@keyword.directive.r',     { fg = colors.blue,      bg = 'NONE' })  -- library, require

  -- Strings
  highlight(0, '@string.r',                { fg = colors.redLight,  bg = 'NONE' })  -- Strings
  highlight(0, '@string.escape.r',         { fg = colors.pink,      bg = 'NONE' })  -- Escape sequences
  highlight(0, '@string.special.r',        { fg = colors.pink,      bg = 'NONE' })  -- Special strings

  -- Numbers
  highlight(0, '@number.r',                { fg = colors.greenLight, bg = 'NONE' })  -- Numbers
  highlight(0, '@number.float.r',          { fg = colors.greenLight, bg = 'NONE' })  -- Floats

  -- Booleans
  highlight(0, '@boolean.r',               { fg = colors.blue,      bg = 'NONE' })  -- TRUE, FALSE

  -- Comments
  highlight(0, '@comment.r',               { fg = colors.red,       bg = 'NONE' })  -- Comments
  highlight(0, '@comment.documentation.r', { fg = colors.red,       bg = 'NONE' })  -- Roxygen documentation

  -- Operators
  highlight(0, '@operator.r',              { fg = colors.white,     bg = 'NONE' })  -- Operators

  -- Punctuation
  highlight(0, '@punctuation.bracket.r',   { fg = colors.white,     bg = 'NONE' })  -- (), [], {}
  highlight(0, '@punctuation.delimiter.r', { fg = colors.white,     bg = 'NONE' })  -- , ;
  ...
  highlight(0, '@lsp.type.variable.r',      { fg = colors.white,     bg = 'NONE' })  -- Variables
  ...
  highlight(0, '@lsp.type.keyword.r',       { fg = colors.blue,      bg = 'NONE' })  -- Keywords
  highlight(0, '@lsp.type.operator.r',      { fg = colors.white,     bg = 'NONE' })  -- Operators
  highlight(0, '@lsp.type.comment.r',       { fg = colors.red,       bg = 'NONE' })  -- Comments
  highlight(0, '@lsp.type.string.r',        { fg = colors.redLight,  bg = 'NONE' })  -- Strings
  highlight(0, '@lsp.type.number.r',        { fg = colors.greenLight, bg = 'NONE' })  -- Numbers

  -- LSP Modifiers
  highlight(0, '@lsp.typemod.variable.readonly.r',  { fg = colors.purple,   bg = 'NONE' })  -- Constants
  ...
  -- Math Functions
  highlight(0, 'rFuncMath',         { fg = colors.orange,     bg = 'NONE' })  -- abs, sqrt, exp, log, sin, cos, tan, etc.

  -- String Functions
  highlight(0, 'rFuncString',       { fg = colors.orange,     bg = 'NONE' })  -- paste, sprintf, nchar, substr, gsub, etc.

  -- Data Manipulation
  highlight(0, 'rFuncData',         { fg = colors.orange,     bg = 'NONE' })  -- c, rep, seq, length, dim, names, etc.

  -- I/O Functions
  highlight(0, 'rFuncIO',           { fg = colors.orange,     bg = 'NONE' })  -- read.csv, write.csv, readLines, etc.

  -- Apply Family
  highlight(0, 'rFuncApply',        { fg = colors.orange,     bg = 'NONE' })  -- apply, lapply, sapply, mapply, tapply, etc.

  -- Type Checking
  highlight(0, 'rFuncIs',           { fg = colors.orange,     bg = 'NONE' })  -- is.null, is.na, is.numeric, etc.

  -- Type Conversion
  highlight(0, 'rFuncAs',           { fg = colors.orange,     bg = 'NONE' })  -- as.numeric, as.character, as.data.frame, etc.

  -- Environment Functions
  highlight(0, 'rFuncEnv',          { fg = colors.orange,     bg = 'NONE' })  -- environment, assign, get, exists, rm, ls, etc.

  -- OOP Functions
  highlight(0, 'rFuncOOP',          { fg = colors.orange,     bg = 'NONE' })  -- class, typeof, mode, setClass, setMethod, etc.
  ...
  -- tidyr
  highlight(0, 'rTidyrFunc',        { fg = colors.orange,     bg = 'NONE' })  -- pivot_longer, pivot_wider, separate, etc.

  -- purrr
  highlight(0, 'rPurrrFunc',        { fg = colors.orange,     bg = 'NONE' })  -- map, map2, pmap, reduce, etc.

  -- stringr
  highlight(0, 'rStringrFunc',      { fg = colors.orange,     bg = 'NONE' })  -- str_c, str_detect, str_replace, etc.

  -- readr
  highlight(0, 'rReadrFunc',        { fg = colors.orange,     bg = 'NONE' })  -- read_csv, read_tsv, write_csv, etc.
  ...
  highlight(0, 'rS4Method',         { fg = colors.orange,     bg = 'NONE' })  -- setMethod
  ...
  highlight(0, 'rFuncStat',         { fg = colors.orange,     bg = 'NONE' })  -- mean, median, sd, var, cor, cov, etc.
  highlight(0, 'rFuncDist',         { fg = colors.orange,     bg = 'NONE' })  -- dnorm, pnorm, qnorm, rnorm, dbinom, etc.
  highlight(0, 'rFuncTest',         { fg = colors.orange,     bg = 'NONE' })  -- t.test, chisq.test, wilcox.test, etc.
  highlight(0, 'rFuncModel',        { fg = colors.orange,     bg = 'NONE' })  -- lm, glm, aov, anova, summary, predict, etc.
  ...
  highlight(0, 'rDataTableFunc',    { fg = colors.orange,     bg = 'NONE' })  -- data.table, setDT, setkey, setorder, etc.
  ...
  highlight(0, 'rmdChunkDelim',     { fg = colors.gray,       bg = 'NONE' })  -- ``` delimiters
